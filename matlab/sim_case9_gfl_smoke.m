%% minimal smoke test for a single zero-power GFL on case9
...
% add one online GFL (zero power references)
...
[outputs,~] = simgrid(ps,event,'sim_case9_gfl_smoke',opt);
...
if ~outputs.success || isnan(t_last) || t_last < (t_max - 1e-6)
    error('sim_case9_gfl_smoke failed: t_last = %.6f, success = %d',t_last,outputs.success);
end
fprintf('sim_case9_gfl_smoke passed: reached t = %.6f s\n',t_last);
