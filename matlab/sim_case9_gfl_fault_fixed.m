%% case9: single GFL with fixed branch-trip disturbance
...
% after branch-trip discrete event, Newton warm-start is more robust than fsolve here
...
% start + fixed branch trip + finish (single deterministic fault event)
event = zeros(3,C.ev.cols);
event(1,[C.ev.time C.ev.type]) = [0 C.ev.start];
event(2,[C.ev.time C.ev.type]) = [0.2 C.ev.trip_branch];
event(2,C.ev.branch_loc) = 6;
event(3,[C.ev.time C.ev.type]) = [t_max C.ev.finish];

% run fixed-fault simulation
[outputs,~] = simgrid(ps,event,'sim_case9_gfl_fault_fixed',opt);
...
if ~outputs.success || isnan(t_last) || t_last < (t_max - 1e-6)
    error('sim_case9_gfl_fault_fixed failed: t_last = %.6f, success = %d',t_last,outputs.success);
end

% verify that at least one exogenous discrete action was recorded
if ~isfield(outputs,'event_record') || isempty(outputs.event_record)
    error('sim_case9_gfl_fault_fixed failed: no events recorded.');
end
if ~any(outputs.event_record(:,C.ev.type) == C.ev.trip_branch)
    error('sim_case9_gfl_fault_fixed failed: fixed branch-trip action was not recorded.');
end

fprintf('sim_case9_gfl_fault_fixed passed: reached t = %.6f s with recorded branch trip event.\n',t_last);
